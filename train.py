...
def main():
    ...
    parser.add_argument('--results_dir', type=str, default='./results/ISGAN',
    ...
    trainer.extend(sample_generate_light(gen, out, rows=10, cols=10),
                   ...
                   priority=extension.PRIORITY_WRITER)
    trainer.extend(calc_inception(gen, n_ims=5000, splits=1,  dst=args.results_dir, path=args.inception_model_path),
                   trigger=(config.evaluation_interval, 'iteration'),
                   priority=extension.PRIORITY_WRITER)
    trainer.extend(calc_FID(gen, n_ims=5000, dst=args.results_dir, path=args.inception_model_path, stat_file=args.stat_file),
        trigger=(config.evaluation_interval, 'iteration'),
        priority=extension.PRIORITY_WRITER)
    trainer.extend(
        monitor_largest_singular_values(dis, dst=args.results_dir),
        trigger=(config.evaluation_interval, 'iteration'),
        priority=extension.PRIORITY_WRITER)
